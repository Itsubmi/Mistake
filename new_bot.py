...
TEXT_3 = "Одна голова хорошо, а две — <b>лучше</b>. Здесь ты найдёшь себе пару, с кем можно <i>покреативить и разогнать идеи</i>."                     # ЗАГЛУШКА
TEXT_4 = "Держи <i>«Выручалочку»</i> — доску объявлений с анкетами креаторов, с которыми можно поштурмить."                     # ЗАГЛУШКА
TEXT_5 = "<b>Важно:</b> брать деньги за штурм нельзя. Вместо оплаты можно предложить ответную креативную сессию."                     # ЗАГЛУШКА
TEXT_6 = "Но, сперва, давай заполним <b>анкету</b>, чтобы лучше узнать твою креативную голову."                     # ЗАГЛУШКА
...
TEXT_8 = "Анкета проверена и добавлена в <i>общую библиотеку.</i>"                     # ЗАГЛУШКА
...
def delete_user(telegram_id):
    ...
    cur.execute("DELETE FROM introductions WHERE user_a = ? OR user_b = ?", (telegram_id, telegram_id))
    conn.commit()
    conn.close()
...
def cmd_start(update: Update, context: CallbackContext) -> int:
    ...
    if is_registered(user.id):
        ...
        keyboard = InlineKeyboardMarkup(
            [
                [InlineKeyboardButton("✅ Продолжить", callback_data="back_to_menu")],
                [InlineKeyboardButton("🗑 Удалить анкету", callback_data="delete_ask_1")],
            ]
        )
        update.message.reply_text("👋 <b>С возвращением!</b> Вот твоя анкета:", parse_mode="HTML")
        send_profile_card(context, update.message.chat_id, user_row, keyboard)
...
def delete_ask_1_callback(update: Update, context: CallbackContext) -> None:
    query = update.callback_query
    query.answer()
    keyboard = InlineKeyboardMarkup(
        [
            [InlineKeyboardButton("❗ Да, удалить", callback_data="delete_ask_2")],
            [InlineKeyboardButton("↩️ Отмена", callback_data="back_to_menu")],
        ]
    )
    context.bot.send_message(
        chat_id=query.from_user.id,
        text="⚠️ Ты уверен, что хочешь удалить свою анкету?\nВсе данные будут стёрты.",
        reply_markup=keyboard,
    )


def delete_ask_2_callback(update: Update, context: CallbackContext) -> None:
    query = update.callback_query
    query.answer()
    keyboard = InlineKeyboardMarkup(
        [
            [InlineKeyboardButton("Да, удалить навсегда", callback_data="delete_confirm", style="danger")],
            [InlineKeyboardButton("Отмена", callback_data="back_to_menu")],
        ]
    )
    context.bot.send_message(
        chat_id=query.from_user.id,
        text="❗️ Восстановить анкету после удаления будет нельзя. Продолжаешь?",
        reply_markup=keyboard,
        parse_mode="HTML",
    )


def delete_confirm_callback(update: Update, context: CallbackContext) -> None:
    query = update.callback_query
    query.answer()
...
def main():
    ...
    dispatcher.add_handler(CallbackQueryHandler(delete_ask_1_callback, pattern="^delete_ask_1$"))
    dispatcher.add_handler(CallbackQueryHandler(delete_ask_2_callback, pattern="^delete_ask_2$"))
    dispatcher.add_handler(CallbackQueryHandler(delete_confirm_callback, pattern="^delete_confirm$"))
